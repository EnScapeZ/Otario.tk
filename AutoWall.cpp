...
bool HandleBulletPenetration2(WeaponInfo_t *wpn_data, FireBulletData &data)
{
	...
	if (data.penetrate_count <= 0 || wpn_data->flPenetration <= 0.f)
		return false;
...
bool CAutoWall::SimulateFireBullet(C_BaseCombatWeapon *weap, FireBulletData &data, C_BaseEntity *player, int hitbox)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
		...
		if (data.trace_length > 3000.f && weaponData->flPenetration > 0.f || enter_surf_penetration_mod < 0.1f)
			break;
		...
		if ((data.enter_trace.hitgroup <= 7) && (data.enter_trace.hitgroup > 0))
		{
			...
			ScaleDamage(data.enter_trace.hitgroup, pPlayer, weaponData->flArmorRatio, data.current_damage);
