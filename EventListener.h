...
public:
	...
	void playerhurt(IGameEvent* game_event)
	{
		...
		{
			Globals::shots_hit[entity->EntIndex()]++;
			...
			switch (g_Settings.iHitSound)
			{
			case 0:
				break;
			case 1:
				hitsound = "buttons\\arena_switch_press_02.wav";
				break;
			case 2:
				hitsound = "bubble.mp3";
				break;
			...
			g_pSurface->IPlaySound(hitsound);
			...
			{
				g_pConVar->ConsoleColorPrintf(Color2(255, 0, 0, 255), "[BORED] ");
			...
			{
				if (health == 0)
					g_pEngine->ExecuteClientCmd("say sub to bored");
			}
		}
	}
	...
	void ItemPurchaseListener(IGameEvent* game_event)
	{
		...
		g_pConVar->ConsoleColorPrintf(Color2(0, 255, 0, 255), "[BORED] ");
