...
void ScrollBar::Render()
{
	...
	/* Up/down button */
	g_Render.RectFilled(this->rcUpButton, style.colMenuControlFill);
	g_Render.RectFilled(this->rcDownButton, style.colMenuControlFill);
...
//	g_Render.RectFilled(rcBoundingBox, style.colMenuStyle);
   ...
   // g_Render.Rect(rcBoundingBox, style.colSectionOutl);
	...
	if (this->bIsActive)
	{
		g_Render.RectFilledGradient(this->rcBoundingBox.Pos2(), { this->rcBoundingBox.right - 2, this->rcBoundingBox.bottom + 3 }, Color(180, 130, 200, 255), Color(180, 130, 200, 255), GradientType::GRADIENT_VERTICAL);
	...
	if (this->bIsHovered)
	{
		g_Render.RectFilledGradient(this->rcBoundingBox.Pos2(), { this->rcBoundingBox.right - 2, this->rcBoundingBox.bottom + 3 }, Color(180, 130, 200, 125), Color(180, 130, 200, 125), GradientType::GRADIENT_VERTICAL);
...
void Checkbox::Render()
{
    ...
    if (*this->bCheckboxValue)
        g_Render.RectFilled(this->rcBox, style.colMenuControlFill);
...
void ComboBox::Render()
{
    ...
    /* Render the small triangle */
    {
        ...
        g_Render.TriangleFilled(ptPosLeft, ptPosRight, ptPosMid, style.colMenuControlFill);
    }/*-------------------------*/
...
void MultiBox::RenderSelectables()
{
	...
	for (int i = 0; i < vecSelectables.size(); i++) {
		...
		g_Render.String({ this->rcBoundingBox.left + 10, this->rcBoundingBox.bottom + this->rcBoundingBox.Height() * i + 10 },
			CD3DFONT_CENTERED_Y, Color(180, 130, 200, 255), pFont.get(), current.c_str());
	}
}
